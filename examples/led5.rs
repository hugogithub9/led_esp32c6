//maintenant on rajoute au modele un bouton dentree selon le temps dappuie genere couleur differente . wait_event
...
mod activator {
    ...
    impl xdevs::Atomic for Activator {
        ...
        fn delta_ext(state: &mut Self::State, e: f64, input: &Self::Input) {
            ...
            let values = input.color.get_values();

            if let Some(c) = values.first() {
                if *c == 'g' {
                    neopixel(Rgb::new(0, 255, 0), &mut state.tx).unwrap();
                    println!("green LED")
                }
                if *c == 'b' {
                    neopixel(Rgb::new(0, 0, 255), &mut state.tx).unwrap();
                    println!("blue LED")
                }
                if *c == 'r' {
                    neopixel(Rgb::new(255, 0, 0), &mut state.tx).unwrap();
                    println!("red LED")
                }
            }

            //if input.color.get_values() = 'g' {
            //neopixel(Rgb::new(0, 255, 0), &mut tx).unwrap();
            //println!("green LED");
            //}
            //if input.color.get_values() = 'b' {
            //neopixel(Rgb::new(0, 0, 255), &mut tx).unwrap();
            //println!("blue LED");
            //}
            //if input.color.get_values() = 'r' {
...
xdevs::component!(
    ...
    couplings = {
        ...
        controller.color -> activator.color,
    }
);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let buttonled = ButtonLED::new(button, controller, activator);
