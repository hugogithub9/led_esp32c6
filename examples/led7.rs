...
mod button {
    use esp_idf_hal::peripherals::Peripherals;
    use esp_idf_hal::{gpio::*, peripherals};
    ...
    pub struct ButtonState {
        ...
        clock: f64,
        pressed: bool,
        tick: PinDriver<'static, Gpio14, Input>, //name of button
    }

    impl ButtonState {
        pub fn new(tick: PinDriver<'static, Gpio14, Input>) -> Self {
            Self {
                sigma: 0.0,
                clock: 0.0,
                pressed: false,
    ...
    impl xdevs::Atomic for Button {
        fn delta_int(state: &mut Self::State) {
            ...
            state.pressed = state.tick.is_high();
        }

        fn lambda(state: &Self::State, output: &mut Self::Output) {
            if state.tick.is_high() && !state.pressed {
                println!("[t={}]: the button is pressed", state.clock + state.sigma);
                output.pulse.add_value(true).unwrap();
            } else if state.tick.is_low() && state.pressed {
                println!(
                    "[t={}]: the button is not pressed",
                    state.clock + state.sigma
                );
                output.pulse.add_value(false).unwrap();
            }
        }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut tick = PinDriver::input(peripherals.pins.gpio14).unwrap();
    tick.set_pull(Pull::Down).unwrap();
